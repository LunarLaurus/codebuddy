...
from util.db_utils import fetch_function_name_and_file


def build_code_map_from_db(conn, commit_sha="HEAD") -> Dict[str, Any]:
    """
    Build a unified code map from the database.
    Includes files, symbols, functions, summaries, and references.
    """
    code_map = {}
    cur = conn.cursor()
    ...
    for path in file_id_to_path.values():
        code_map[path] = {"functions": [], "structs": [], "typedefs": [], "globals": []}
    ...
    for cid, calid in cur.fetchall():
        if cid in func_map and calid in func_map:
            c_name, _ = fetch_function_name_and_file(conn, cid)
            cal_name, _ = fetch_function_name_and_file(conn, calid)
    ...
    # Unify duplicate functions (same file + name + lines)
    unify_map = {}
    for fobj in func_map.values():
